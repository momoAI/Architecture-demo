...
class NewsViewModel {
    ...
    private var page = 0
    private var itemList: Array<NewsCellViewModel> = [] {
        didSet {
            self.dataOnChanged?(())
        }
    }
    ...
    /// 添加新的数据 （如上拉加载更多）
    func append(newItems: [NewsCellViewModel]) {
    ...
    func fetchAllDatas() {
        ...
        AF.request("https://c.3g.163.com/nc/article/list/T1467284926140/0-20.html").responseJSON { rsp in
            ...
            case .success(let json):
                ...
                self.itemList = newsJsonArr.map { return NewsCellViewModel(item: News($0)) }
    ...
    func fetchPartDatas() {
        ...
        AF.request("https://c.3g.163.com/nc/article/list/T1467284926140/\(page * pageSize)-\((page + 1) * pageSize).html").responseJSON { rsp in
            ...
            case .success(let json):
                let jsonData = JSON.init(json)
                let newsJsonArr = jsonData["T1467284926140"].arrayValue
                let newDatas = newsJsonArr.map { return NewsCellViewModel(item: News($0)) }
